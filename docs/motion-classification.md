...
- `mounted_off.csv`: same mounted position with both machines off,
...
- The production local detector is a power/cadence controller only. After five
  ...
  This is not a guarantee when uploads fail. Initial startup allowance stays five
...
The mounted-off validation and real dryer-to-quiet transition are captured. Do
not OTA during a load. The running dryer capture confirms a large separation from
both stationary baselines, but gentle washer motion remains unproven.
